...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
			 array<array<String^>^> ^file=gcnew array<array<String^>^>(10);
			 ...
			 bp->train(x,y,count);
			 MessageBox::Show("训练成功！","训练成功！");
		 }
		 array<double> ^calx(String ^filename)
		 {
			array<double> ^ans=gcnew array<double>(18);
			Bitmap ^bit=gcnew Bitmap(filename);
			for (int k=0;k<=ans->Length-1;k++) ans[k]=0;
			for (int k=0;k<=bit->Height-1;k++)
				for (int l=0;l<=bit->Width-1;l++)
				{
					Color ^tt=bit->GetPixel(l,k);
					if (bit->GetPixel(l,k).R<=128)
					{
						ans[l*9/bit->Width]+=(double)1*9/bit->Height/bit->Width;
						ans[9+k*9/bit->Height]+=(double)1*9/bit->Height/bit->Width;
					}
				}
			return ans;
			
		 }
		 array<double> ^caly(int value)
		 {
				array<double> ^ans=gcnew array<double>(10);
				for (int k=0;k<=9;k++) ans[k]=0;
				ans[value]=1;
				return ans;
		 }
private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) {
			 array<int> ^conf={18,20,10};
			 bp=gcnew Bpnet;
			 bp->init(conf);
		 }
private: System::Void button2_Click(System::Object^  sender, System::EventArgs^  e) {
			 if (textBox1->Text!="")
			 {
				 array<double> ^y=bp->identify(calx(textBox2->Text));
				 int ans=0;
				 double max=y[0];
				 for (int i=1;i<=9;i++)
					 if (max<y[i])
