classdef kfacility
    ...
    methods
        ...
        function [res,flag]=findns(this,S,p,e,deltaflag)%寻找下一个符合条件的选址集合 S：当前选址集合 p：交换个数 deltaflag决定代价计算的方式
            NIS=setdiff(this.F,S);
            costS=this.calcost(S,p,deltaflag);
            if length(S)<this.k
                for i=NIS
                    ts=union(S,i);
                    costts=this.calcost(ts,p,deltaflag);
        ...
        function [cost,res]=bestcost(this,S0,p,e,deltaflag) %局部搜索局部解 S：当前选址集合 p：交换个数 deltaflag决定代价计算的方式
            flag=1;
            S=S0;
            while flag==1
                [S,flag]=this.findns(S,p,e,deltaflag);
                %showcost=this.calcost(S,p,deltaflag)
                %showdeltaflag=deltaflag
            end;
            cost=this.calcost(S,p,0);
            res=S;
        end;
        
        function [cost,res]=multical(this,times,p,flag) %多次采样计算局部解 times采样次数 p：交换个数 flag：是否与delta=1的情况比较
            e=0.001;
            cost=inf;
            res=[];
            for i=1:times
                ...
                [tcost,tres]=this.bestcost(S,p,e,1);
                %1
                if (tcost<cost)
                    cost=tcost;
                    res=tres;
                ...
                if flag==1
                    [tcost,tres]=this.bestcost(S,p,e,0);
                    %0
                    if (tcost<cost)
                        cost=tcost;
                        res=tres;
